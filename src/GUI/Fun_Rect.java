 ...
 * Time: 13:19
 */
public class Fun_Rect extends Region {
    ...
    private void initGraphics() {
        ring   = new Rectangle(0, 0, PREFERRED_WIDTH, PREFERRED_HEIGHT);
        ...
        mask = new Rectangle(PREFERRED_WIDTH*.05, PREFERRED_HEIGHT * .05, PREFERRED_WIDTH * .9, PREFERRED_HEIGHT * .9);
        canvas.setClip(mask);
    ...
    public void setLevel(final double LEVEL) {
        ...
        text.setFill(LEVEL < 0.45 ? Color.BLACK : Color.BLACK);
    }
    ...
    private void resize() {
        ...
        if (width > 0 && height > 0) {
            ...
            mask.setX(width * 0.05);
            mask.setY(height * 0.05);
            mask.setWidth(width * .9);
            mask.setHeight(height * .9);
            ...
            for( int i = 0 ; i < detail + 1 ; i++ ) {
                ...
                p.y = height * (1d - gauge.getCurrentValue());
